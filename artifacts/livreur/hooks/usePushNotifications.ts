...
async function getExpoPushToken(): Promise<string | null> {
  try {
    ...
    const existing = existingPermissions.granted ?? existingPermissions.status === "granted";
    const requestedPermissions = async () =>
    ...
    const finalStatus =
      existing
        ? existing
        : ((await requestedPermissions()).granted ??
          (await requestedPermissions()).status === "granted");
